...
const Inbox: React.FC = () => {
  ...
  const [serverHealth, setServerHealth] = useState<'up' | 'down' | 'checking'>('checking');
  ...
  const playNotification = () => {
    if (isSoundEnabled && audioRef.current) {
      audioRef.current.play().catch(e => console.warn("Áudio bloqueado."));
    }
  };
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'wb_contacts') loadContacts();
      if (e.key === 'wb_incoming') setMessages(JSON.parse(e.newValue || '[]'));
    };
  ...
  const fetchMessages = async (forceResync = false) => {
    ...
    // Garante que a URL termine corretamente com /messages
    let baseUrl = config.bridgeUrl.trim();
    if (baseUrl.endsWith('/')) baseUrl = baseUrl.slice(0, -1);
    const dataUrl = baseUrl.endsWith('/messages') ? baseUrl : `${baseUrl}/messages`;
    ...
    try {
      ...
      if (Array.isArray(rawData)) {
        const formattedMessages: IncomingMessage[] = rawData.map((m: any) => ({
          id: m.id || `${m.from}-${m.timestamp}-${Math.random()}`,
          from: m.from || 'Sistema',
        ...
        const localSaved = forceResync ? [] : JSON.parse(localStorage.getItem('wb_incoming') || '[]');
        const existingIds = new Set(localSaved.map((m: any) => m.id));
        const newMessages = formattedMessages.filter(m => !existingIds.has(m.id));

        if (newMessages.length > 0 || forceResync) {
          // Tocar som se houver mensagens de clientes
          if (newMessages.some(m => !m.isMe) && !forceResync) {
            playNotification();
          }

          const updated = forceResync ? formattedMessages : [...localSaved, ...newMessages];
          // Ordenar por data
  ...
  useEffect(() => {
    fetchMessages(); // Busca imediata ao abrir
    
    // Polling a cada 4 segundos para maior agilidade
  ...
  const getDisplayName = (phone: string) => {
    ...
    if (lastMessageWithName?.fromName) return { name: lastMessageWithName.fromName, isSaved: false };
      ...
      case 'image':
        return (
          <div className="space-y-2">
            <img src={msg.mediaUrl} alt="Mídia" className="rounded-lg max-w-full h-auto cursor-pointer hover:opacity-90 border border-black/5" onClick={() => window.open(msg.mediaUrl, '_blank')} />
  ...
  return (
    <div className="bg-white rounded-none md:rounded-2xl border-0 md:border border-slate-200 shadow-sm overflow-hidden h-screen md:h-[calc(100vh-200px)] flex flex-col">
      {/* Header de Status e Diagnóstico */}
      <div className="px-4 py-3 bg-slate-900 flex justify-between items-center border-b border-slate-800">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${serverHealth === 'up' ? 'bg-emerald-500 animate-pulse' : serverHealth === 'checking' ? 'bg-amber-500 animate-bounce' : 'bg-rose-500'}`}></div>
            <span className="text-[9px] text-white font-bold uppercase tracking-widest">
              {serverHealth === 'up' ? 'Conectado' : serverHealth === 'checking' ? 'Verificando...' : 'Desconectado'}
            </span>
          </div>
          <div className="hidden sm:block text-[9px] text-slate-500 font-bold uppercase">
            Última busca: {lastCheck}
          </div>
          <button 
        ...
        <button 
          ...
          className="text-[9px] font-bold bg-emerald-500 text-white px-3 py-1.5 rounded-lg hover:bg-emerald-600 uppercase shadow-lg shadow-emerald-500/20 active:scale-95 transition-all"
        >
        ...
        <div className={`${selectedChat ? 'hidden md:flex' : 'flex'} w-full md:w-80 border-r border-slate-100 flex-col bg-white overflow-y-auto`}>
          {sortedChats.length === 0 ? (
            <div className="p-10 text-center opacity-20 mt-10">
              ...
              <p className="text-[10px] font-bold uppercase">Nenhuma conversa ativa</p>
              <p className="text-[8px] mt-2 uppercase font-medium">Envie um "Olá" para o número de teste</p>
            </div>
          ) : (
        ...
        <div className={`${!selectedChat ? 'hidden md:flex' : 'flex'} flex-1 flex-col bg-[#efeae2] relative shadow-inner`}>
          {selectedChat ? (
            <>
              <div className="p-4 bg-white border-b border-slate-200 flex justify-between items-center z-10 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-mono">+{selectedChat}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-white border-t border-slate-200 pb-safe">
                <div className="flex gap-2 items-end max-w-4xl mx-auto">
                  <button onClick={() => setIsMediaModalOpen(true)} className="w-12 h-12 bg-slate-100 text-slate-500 rounded-full flex items-center justify-center hover:bg-slate-200 transition-colors flex-shrink-0">
                    📎
                  </button>
                  <textarea 
                    value={replyText}
                  ...
                  />
                  <button onClick={() => handleSendReply()} disabled={!replyText.trim() || isSendingReply} className="w-12 h-12 bg-emerald-500 text-white rounded-full flex items-center justify-center hover:bg-emerald-600 disabled:opacity-50 transition-all shadow-md active:scale-95 flex-shrink-0">
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-12 text-center">
              <div className="w-24 h-24 bg-white/50 rounded-full flex items-center justify-center text-4xl mb-4 grayscale opacity-30 shadow-sm animate-pulse">💬</div>
              <h3 className="font-bold text-slate-500 uppercase tracking-widest text-xs">Aguardando Mensagens</h3>
              <p className="text-[10px] mt-2 max-w-[240px] leading-relaxed">Verifique se sua URL da Ponte está correta nas configurações e se o servidor no Render está ativo.</p>
              {!isSoundEnabled && (
                <button 
                  onClick={() => setIsSoundEnabled(true)}
                  className="mt-6 px-6 py-3 bg-emerald-500 text-white rounded-xl font-bold text-xs shadow-lg shadow-emerald-100 hover:scale-105 transition-all"
                >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl border border-slate-100">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-[9px] font-bold text-slate-400 uppercase">URL do Arquivo</label>
