...
const Inbox: React.FC = () => {
    ...
    const poll = setInterval(async () => {
      ...
      try {
        ...
        if (data.entry?.[0]?.changes?.[0]?.value?.messages) {
          newMsgs = data.entry[0].changes[0].value.messages;
  ...
  const processMessages = (rawList: any[]) => {
    // Fix: Added explicit return type (IncomingMessage | null) to ensure map output is compatible with the filter type predicate
    const processed: IncomingMessage[] = rawList.map((m: any): IncomingMessage | null => {
    ...
    const added = processed.filter(p => !existingIds.has(p.id));

    if (added.length > 0) {
      const updated = [...current, ...added].sort((a, b) => 
        ...
        {selectedChat ? (
          <>
            ...
            <div className="p-4 bg-[#f0f2f5] flex gap-2">
              <input value={replyText} onChange={e => setReplyText(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSend()} className="flex-1 px-4 py-2 rounded-full border-0 outline-none" placeholder="Digite uma mensagem" />
