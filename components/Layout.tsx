...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (messagesRaw) {
        ...
        if (Array.isArray(messages)) {
          ...
          document.title = count > 0 ? `(${count}) WhatsJhonTechAI` : `WhatsJhonTechAI`;
        }
      }
    } catch(e) {}
  };
  ...
  const navItems = [
    ...
    { name: 'Robô', path: '/automation', icon: '🤖' },
    { name: 'Envios', path: '/campaigns', icon: '🚀' },
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className="flex-1 md:ml-64 p-0 md:p-8">
        <header className="hidden md:flex justify-between items-center mb-8 bg-white/50 backdrop-blur-md px-6 py-4 rounded-2xl border border-slate-200/50">
          <div>
            <h1 className="text-lg font-bold text-slate-800">
              {navItems.find(item => item.path === location.pathname)?.name || 'Painel'}
            </h1>
            {senderInfo?.phone && (
              <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-bold">CONECTADO: +{senderInfo.phone}</span>
            )}
          </div>
        </header>
        {children}
